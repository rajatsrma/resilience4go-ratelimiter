...
func (s SlidingWindowCounterRatelimiter) CheckForRateLimit(request Request) Response {
	...
	currentTime, filteredTimestamps := getRequestInTimeWindow(timestampSlice, s.timeWindowInSeconds)
	...
	if len(filteredTimestamps) > int(s.allowedRequestsCount) {
		...
		return Response{RequestThrottled: false, AvailableRequestQuota: int64(len(filteredTimestamps) - int(s.allowedRequestsCount))}
	}
}
...
func getRequestInTimeWindow(timestamps []string, timeWindowInSeconds int64) (int64, []string) {
	currentTime := time.Now().UnixNano()
	timeWindowStart := currentTime - (timeWindowInSeconds * 1000 * 1000)

	// check number of request within these limits
	filteredRequests := filter(timestamps, func(timestamp string) bool {
	...
	return currentTime, filteredRequests
}
